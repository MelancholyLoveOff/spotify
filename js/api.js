...
const initializeData = (data) => {
    try {
        ...
        db.artists.forEach(artist => {
            ...
            let ouvintesDoHype = 0;
            let ouvintesDoLegado = 0;
            let ouvintesDiarios = 0;
            
            artistSongs.forEach(song => {
                const streamsTotais = parseInt(song.totalStreams) || 0;
                const streamsDiarios = parseInt(song.streams) || 0;
                
                // 1. O PODER DA DIVULGAÇÃO DIÁRIA
                // Se a pessoa gastou a Ação pra divulgar hoje, 80% do tráfego vira ouvinte único!
                ouvintesDiarios += Math.floor(streamsDiarios * 0.80);
                
                // 2. SISTEMA ANTI-INFLAÇÃO (A Mágica dos 30 Dias)
                if (song.parentReleaseDate) {
                    const releaseDate = new Date(song.parentReleaseDate);
                    const daysOld = (now - releaseDate) / (1000 * 60 * 60 * 24);
                    
                    if (daysOld >= 0 && daysOld <= 30) {
                        // MÚSICA FRESCA (No Hype)
                        if (streamsTotais <= 50000000) {
                            // Hit normal: 15% vira ouvinte
                            ouvintesDoHype += Math.floor(streamsTotais * 0.15);
                        } else if (streamsTotais <= 300000000) {
                            // Smash Hit: Passou de 50M, o Spotify entende que tem fã dando repeat. A conversão cai pra 5%.
                            ouvintesDoHype += 7500000; // Máximo da faixa anterior
                            ouvintesDoHype += Math.floor((streamsTotais - 50000000) * 0.05); 
                        } else {
                            // Mega Hit Global (>300M): O grande Nerf. Acima disso, apenas 1% vira ouvinte novo.
                            ouvintesDoHype += 20000000; // Máximo acumulado até aqui
                            ouvintesDoHype += Math.floor((streamsTotais - 300000000) * 0.01); 
                        }
                    } else {
                        // CATÁLOGO ANTIGO (Mais de 30 dias) -> 0.1% de conversão
                        ouvintesDoLegado += Math.floor(streamsTotais * 0.001); 
                    }
                } else {
                    // Sem data configurada -> Assume que é catálogo antigo
                    ouvintesDoLegado += Math.floor(streamsTotais * 0.001);
                }
            });
            
            // Soma total vitalícia
            artist.careerTotalStreams = artistSongs.reduce((sum, song) => sum + (parseInt(song.totalStreams) || 0), 0);
            
            // 3. FANDOM FIXO
            // C) Força da Imagem e Fandom
            const pontosDeImagem = parseInt(artist.personalPoints) || 150;
            let ouvintesFieis = Math.floor(pontosDeImagem * 0.5) * 1000;

            // --- NOVO: FATOR DISCOGRAFIA ---
            // Conta quantos lançamentos o artista tem
            const totalLancamentos = artist.albums.length + artist.singles.length;

            if (totalLancamentos >= 8) {
                ouvintesFieis *= 1.3; // +30% de bônus para veteranos com discografia vasta
            } else if (totalLancamentos >= 3) {
                ouvintesFieis *= 1.1; // +10% de bônus para quem tem discografia em crescimento
            }
            
            // RESULTADO FINAL
            artist.monthlyListeners = ouvintesFieis + ouvintesDoLegado + ouvintesDoHype + ouvintesDiarios;
        });
